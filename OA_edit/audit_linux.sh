...
url="http://openaudit.sai.gameloft.org/open-audit/index.php/system/add_system"
		...
		for san in $(/opt/IBM_DS/client/SMcli -d | grep -v -e '^$' | grep -v 'SMcli' | cut -f2 ); do
			...
			if [ "$submit_online" = "y" ]; then
				...
				if [ -n "$(which wget 2>/dev/null)" ]; then
					...
					wget --no-proxy --delete-after --post-file=/tmp/"$san".txt "$san_url" 2>/dev/null
				else
					if [ -n "$(which curl 2>/dev/null)" ]; then
					if [ "$debugging" -gt 1 ]; then
						echo "Sending using curl."
					fi
						curl --noproxy '*' --data @/tmp/"$san".txt "$san_url"
					fi
				fi
			fi
...
if [ "$submit_online" = "y" ]; then
	...
	if [ -n "$(which wget 2>/dev/null)" ]; then
		if [ "$debugging" -gt 3 ]; then
			wget --no-proxy -O add_system --post-file="$xml_file" "$url"
			cat add_system
			rm add_system
		else
			wget --no-proxy --delete-after --post-file="$xml_file" "$url" 2>/dev/null
		fi
	else
		if [ -n "$(which curl 2>/dev/null)" ]; then
			if [ "$debugging" -gt 3 ]; then
				curl --noproxy '*' -o add_system --data "@$xml_file" "$url"
				cat add_system
				rm add_system
			else
				curl --noproxy '*' --data "@$xml_file" "$url"
			fi
		fi
	fi
fi
